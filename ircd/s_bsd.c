...
FdAry	fdas, fdall;
int	highest_fd = 0, readcalls = 0, udpfd = -1, resfd = -1, adfd = -1, isfd = -1;
time_t	timeofday;
...
aClient *cptr;
{
	...
	if (cptr && (IsConnecting(cptr) || IsHandshake(cptr)) &&
		cptr->serv && cptr->serv->by[0])
	{
		bysptr = find_client(cptr->serv->by, NULL);
 ...
 */
int	inetport(cptr, ip, ipmask, port, dolisten)
aClient	*cptr;
char	*ipmask, *ip;
int	port;
int	dolisten;
...
aConfItem *aconf;
{
	...
	if ((cptr = find_listener (aconf->port)))
		...
		if (!IsIllegal(cptr->confs->value.aconf))
			return (-1);
		...
		if (!aconf->passwd || !*aconf->passwd ||
			...
			return (-1);	/* abort on incorrect charset */
		/* but it was just changed, reuse it then */
...
#endif
	if (cptr->fd == -1)	/* open_listener() failed! */
		return (-1);
	return 0;
}
 ...
 * close_listeners
 *
 * Close and free all clients which are in listening state and already 
 * marked as ILLEGAL and have no clients using corresponding configuration
 * entry.  
 * Unix sockets have the path to the socket unlinked for cleanliness.
 */
...
aClient *find_listener(int port)
{
	...
	for (i = highest_fd; i >= 0; i--)
		...
		if (cptr == &me || !IsListener(cptr))
			continue;
		if (port == cptr->confs->value.aconf->port)
			return cptr;
	    }

	return NULL; /* listener not found */
}
#endif
...
void	open_listener(aClient *cptr)
{
...
#endif
	{
		...
		if (!aconf->passwd || !*aconf->passwd ||
		    !(cptr->conv = conv_get_conversion(aconf->passwd)))
		{
			/* bad charset so abort listener for that port */
			cptr->fd = -1;
			return;
		}
...
start_iserv()
{
	...
	if ((time(NULL) - last) > 90)
	    {
		sendto_flag(SCH_ISERV, "Starting iserv...");
		last = time(NULL);
		iserv_spawn += 1;
	    }
	else
	...
	if (socketpair(AF_UNIX, SOCK_STREAM, 0, sp) < 0)
	    {
		sendto_flag(SCH_ERROR, "socketpair() failed!");
		sendto_flag(SCH_ISERV, "Failed to restart iserv!");
	    }
	isfd = sp[0];
	set_non_blocking(sp[0], NULL);
	set_non_blocking(sp[1], NULL);
	val = ISERV_BUFFER;
	if (setsockopt(sp[0], SOL_SOCKET, SO_SNDBUF, (void *) &val,
	    sizeof(val)) < 0)
			sendto_flag(SCH_ISERV,
			    "ISERV_BUFFER too big for sp0 sndbuf, using default");
	if (setsockopt(sp[1], SOL_SOCKET, SO_RCVBUF, (void *) &val,
	    sizeof(val)) < 0)
			sendto_flag(SCH_AUTH,
			    "ISERV_BUFFER too big for sp1 rcvbuf, using default");
	switch (vfork())
	    {
		case -1:
			sendto_flag(SCH_ERROR, "vfork() failed!");
		...
		case 0:
			...
			if (execl(ISERV_PATH, ISERV, NULL) < 0)
				_exit(-1); /* should really not happen.. */
		default:
			close(sp[1]);
	    }
...
#if defined(RLIMIT_FD_MAX) || defined(SOLARIS_10UP)
	struct rlimit limit;
...
void close_client_fd(aClient *cptr)
{
	...
	if ((i = cptr->fd) >= 0)
	{
		...
		local[i] = NULL;
#ifdef USE_SSL
    		if(IsSSL(cptr) && cptr->ssl) /* This is client only */
		{
...
#ifdef USE_SSL
    if (IsSSLListener(cptr))
    {
        extern SSL_CTX *ctx;
        acptr->ssl = NULL;
        /*SSL initialization for client*/
        if((acptr->ssl = SSL_new(ctx)) == NULL)
	{
	    sendto_flag(SCH_LOCAL, "SSL_new failed for client [%s]",
				acptr->sockhost);
			add_connection_refuse(fd, acptr, 0);
			return NULL;
	}
        ...
        SetSSL(acptr);
		Debug((DEBUG_DEBUG,
			"Setting SSL mode for client %s", acptr->sockhost));
    }
#endif
	acptr->fd = fd;
...
ssl_refuse:
	...
	else /* it's me - unix connection or oper mode? */
	    acptr->conv = conv_get_conversion(NULL);
#else
...
#ifdef	UNIXPORT
static	aClient	add_unixconnection(cptr, fd)
aClient	*cptr;
int	fd;
{
	aClient *acptr;
...
aClient *cptr;
{
	...
	while (max--)
		...
		*/
...
#ifdef INET6
			(void)sendto(fdnew, "ERROR :All connections in use\r\n",
				    32, 0, 0, 0);
#else
		...
		istat.is_unknown++;
		/* Notice on connect. */
		sendto_one(acptr, rpl_str(RPL_HELLO, ME), HELLO_MSG);
	}	    
}
...
int	msg_ready;
{
	...
	if (msg_ready &&
	    ...
	    {
		errno = 0;
		length = RECV(cptr, readbuf, sizeof(readbuf));
...
#ifdef USE_SSL
					if (IsSSL(cptr) && strerror_ssl &&
					      strcmp(strerror_ssl, "Success")) {
					    (void)(exit_client(cptr, cptr, &me,
						strerror_ssl));
					    strerror_ssl = NULL;
					}
...
#ifdef INET6
		inetpton(AF_INET6, aconf->passwd, mysk.sin6_addr.s6_addr);
#else
...
#ifdef INET6
		inetpton(AF_INET6, aconf->passwd,from.sin6_addr.s6_addr);
#else
