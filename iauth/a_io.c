 ...
 *   $Id: a_io.c,v 1.9 2010-11-10 13:38:39 gvs Exp $
 */
		...
		case 'O': /* old connection: do nothing, just update data */
			if (cldata[cl].state & A_ACTIVE)
				...
				/* this is not supposed to happen!!! */
                                sendto_log(ALOG_IRCD, LOG_CRIT,
			   "A: Entry %d [%c] is already active (fatal)!",
					   cl, chp[0]);
				exit(1);
			    }
			if (cldata[cl].instance || cldata[cl].rfd > 0 ||
			    ...
			    {
				sendto_log(ALOG_IRCD, LOG_CRIT,
				   "B: Entry %d [%c] is already active! (fatal)",
					   cl, chp[0]);
				exit(1);
			    }
 ...
 */
static void	set_non_blocking(fd, ip, port)
int fd;
char *ip;
u_short port;
{
...
#if defined(INET6)
	inetpton(AF_INET6, ourIP, sk.sin6_addr.s6_addr);
#else
...
#if defined(INET6)
	inetpton(AF_INET6, theirIP, sk.sin6_addr.s6_addr);
#else
