 ...
 *   $Id: ircdwatch.c,v 1.8 2010-11-10 13:38:39 gvs Exp $
 */
...
int spawn (char *cmd) 
{
  ...
  if (pid == 0) {
  /*
   * Signal SIGCHLD should be set to SIG_DFL here to be inherited properly
   */
...
void daemon_run () 
{
  ...
  if (i == -1) {
    /* unable to open pid file.  wrong user? */
    fprintf(stderr, "ircd pid file exists but is unreadable\n");
    exit(1);
  } else  if (!i) {
